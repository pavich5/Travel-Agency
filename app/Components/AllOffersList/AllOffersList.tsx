...
const AllOffersList = ({
  ...
  return (
    <>
          ...
          filteredVacations.map((vacation: any) =>
            vacation.offers.map((offer: any, offerIndex: number) => {
              const isPriceFiltered =
                priceFilter === undefined || offer.totalCost <= priceFilter;
              const isCityFiltered =
              ...
              return (
                isOfferVisible && (
                  <div
                    onClick={() =>
                      // @ts-ignore
                      router.push(`/offer/${offer.id}`, { shallow: true })
                    }
                  ...
                  >
                    <div>
                      ...
                      <button className={styles.viewOfferButton}>
                        View Offer
                      </button>
                    </div>
                  </div>
      ...
      {filteredVacations.length === 0 && <Empty className={styles.noData} />}
    </>
  );
};
