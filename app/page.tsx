...
const LandingPage = () => {
  ...
  const currentItems = vacationsCategories.categories.flatMap(category => category.countrys)
  useEffect(() => {
    ...
    <div className={styles.homePageWrapper}>
      <div>
        <div className={styles.mainSection}>
          <LandingSection filteredData={filteredData} handleInputChange={handleInputChange} searchValue={searchValue} />
        </div>
        <TravelSeasonSection vacationsCategories={vacationsCategories} />
        <TravelDestinationList currentItems={currentItems.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)} />
        <div className={styles.pagination}>
          <LeftOutlined onClick={handlePreviousPage} className={currentPage === 1 ? styles.disabled : ''} />
          {renderPageNumbers()}
          <RightOutlined onClick={handleNextPage} className={currentPage * itemsPerPage >= filteredData.length ? styles.disabled : ''} />
        </div>
